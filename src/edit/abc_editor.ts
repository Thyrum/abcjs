...
const SynthController = require('../synth/synth-controller');
const supportsAudio = require('../synth/supports-audio');
const renderAbc = require('../api/abc_tunebook_svg');
const EditArea = require('./abc_editarea');
...
const Editor = function (editarea, params) {
...
Editor.prototype.redrawMidi = function () {
	if (this.generate_midi && !this.midiPause) {
		const event = new window.CustomEvent("generateMidi", {
			detail: {
				tunes: this.tunes,
	...
	if (this.synth) {
		const userAction = this.synth.synthControl; // Can't really tell if there was a user action before drawing, but we assume that if the synthControl was created already there was a user action.
...
Editor.prototype.setDirtyStyle = function (isDirty) {
	...
	const removeClassName = function (element, className) {
		element.className = parseCommon.strip(element.className.replace(
			new RegExp("(^|\\s+)" + className + "(\\s+|$)"), ' '));
		return element;
	};

	const readonlyClass = 'abc_textarea_dirty';
	const el = this.editarea.getElem();
	if (isDirty) {
