...
function addLineBreaks(lines, linesBreakElements, barNumbers) {
	...
	const outputLines = [];
	const lastKeySig = []; // This is per staff - if the key changed then this will be populated.
	const lastStem = [];
...
function oneTry(measureWidths, idealWidths, accumulator, lineAccumulator, lineWidths, lastVariance, highestVariance, currLine, lineBreaks, startIndex, otherTries) {
	for (let i = startIndex; i < measureWidths.length; i++) {
		...
		const varianceIsClose = Math.abs(thisVariance - lastVariance) < idealWidths[0] / 10; // see if the difference is less than 10%, if so, run the test both ways.
		if (varianceIsClose) {
...
function optimizeLineWidths(widths, lineBreakPoint, lineBreaks, explanation) {
	...
	const numLines = Math.ceil(widths.total / lineBreakPoint); // + 1 TODO-PER: this used to be plus one - not sure why
...
function calcLineWraps(tune, widths, params) {
	...
	for (let s = 0; s < widths.length; s++) {
		...
		const minLineSize = usableWidth / maxSpacing / scale;
