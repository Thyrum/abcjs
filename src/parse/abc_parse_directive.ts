...
(function() {
	...
	const getFontParameter = function(tokens, currentSetting, str, position, cmd) {
		...
		function processNumberOnly() {
			const size = parseInt(tokens[0].token);
			tokens.shift();
			if (!currentSetting) {
		...
		const psFont = fontTranslation(face);
		const font = {};
		if (psFont) {
	...
	const interpretPercMap = function(restOfString) {
		const tokens = restOfString.split(/\s+/); // Allow multiple spaces.
	...
	const midiCmdParam1Integer1OptionalInteger = [
		"program"
	];
	const midiCmdParam2Integer = [
		"ratio",
		"snt",
		"bendvelocity",
		"pitchbend",
	...
	const midiCmdParam4Integer = [
		"beat"
	];
	const midiCmdParam5Integer = [
		"drone"
	];
	const midiCmdParam1String1Integer = [
		"portamento"
	];
	const midiCmdParamFraction = [
		"expand",
		"grace",
		"trim"
	];
	const midiCmdParam1StringVariableIntegers = [
		"drum",
		"chordname"
	];
	const midiCmdParam1Integer1OptionalString = [
		"bassprog", "chordprog"
	];
	...
	parseDirective.addDirective = function(str) {
		const tokens = tokenizer.tokenize(str, 0, str.length);	// 3 or more % in a row, or just spaces after %% is just a comment
			...
			case "setfont":
				...
				if (sfTokens.length >= 4) {
					if (sfTokens[0].token === '-' && sfTokens[1].type === 'number') {
						const sfNum = parseInt(sfTokens[1].token);
						if (sfNum >= 1 && sfNum <= 9) {
