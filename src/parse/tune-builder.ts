const parseKeyVoice = require('../parse/abc_parse_key_voice');
//var parseCommon = require('../parse/abc_common');
//var parseDirective = require('./abc_parse_directive');

const TuneBuilder = function (tune) {
	const self = this;
	const voiceDefs = {}
	...
	this.cleanUp = function (barsperstaff, staffnonote, currSlur) {
		...
		if (hadOverlays) {
			...
			let voiceNum = 0
			let isUseful = voiceUseful(tune.lines, voiceNum)
		...
		for (var i = 0; i < tune.lines.length; i++) {
			const staff = tune.lines[i].staff;
			if (staff) {
				for (tune.staffNum = 0; tune.staffNum < staff.length; tune.staffNum++) {
					...
					for (tune.voiceNum = 0; tune.voiceNum < staff[tune.staffNum].voices.length; tune.voiceNum++) {
						const voice = staff[tune.staffNum].voices[tune.voiceNum];
						...
						if (voice.length > 0 && voice[voice.length - 1].barNumber) {
							...
							const nextLine = getNextMusicLine(tune.lines, i);
							if (nextLine)
	...
	this.appendStartingElement = function (type, startChar, endChar, hashParams2) {
		...
		const staff = tune.lines[tune.lineNum].staff
		if (!staff) return
	...
	this.getCurrentVoice = function () {
		...
		const currLine = getPrevMusicLine(tune.lines, tune.lineNum)
		if (!currLine)
			return null;
		const currStaff = currLine.staff[tune.staffNum];
		if (!currStaff)
			return null;
...
function noWarnBeforeTitle(tune) {
	for (let i = 1; i < tune.lines.length; i++) {
		const line = tune.lines[i]
		if (line.subtitle) {
			const previousLine = tune.lines[i-1]
			if (previousLine.staff) {
				for (let j = 0; j < previousLine.staff.length; j++) {
					for (let v = 0; v < previousLine.staff[j].voices.length; v++) {
						const voice = previousLine.staff[j].voices[v]
						if (voice[voice.length-1].el_type === "key") {
							...
							let nextLine = tune.lines[i]
							while (i < tune.lines.length && !nextLine.staff) {
								i++
								nextLine = tune.lines[i]
							}
							if (nextLine) {
								for (let jj = 0; jj < nextLine.staff.length; jj++) {
									const accidentals = nextLine.staff[jj].key.accidentals
									const newAccidentals = []
									for (let a= 0; a < accidentals.length; a++) {
