...
function accidentalChange(origPitch, newPitch, accidental, origKeySig, newKeySig) {
	const origPitchLetter = pitchToLetter[(origPitch + 49) % 7]; // Make sure it is a positive pitch before normalizing.
	...
	const newPitchLetter = pitchToLetter[(newPitch + 49) % 7]; // Make sure it is a positive pitch before normalizing.
	let newAccidental = 0;
