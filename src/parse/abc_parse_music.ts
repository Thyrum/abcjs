const parseKeyVoice = require('./abc_parse_key_voice');
const transpose = require('./abc_transpose');
...
const MusicParser = function(_tokenizer, _warn, _multilineVars, _tune, _tuneBuilder, _header) {
...
const isInTie = function(multilineVars, overlayLevel, el) {
	...
	const voiceIndex = multilineVars.currentVoice ? multilineVars.currentVoice.staffNum * 100 + multilineVars.currentVoice.index : 0;
...
MusicParser.prototype.parseMusic = function(line) {
	...
	let i = 0;
	const startOfLine = multilineVars.iChar;
	...
	let delayStartNewLine = multilineVars.start_new_line;
	...
	const retHeader = header.letter_to_body_header(line, i);
	if (retHeader[0] > 0) {
		i += retHeader[0];
	...
	while (i < line.length)
	{
		...
		const retInlineHeader = header.letter_to_inline_header(line, i, delayStartNewLine);
		if (retInlineHeader[0] > 0) {
			...
			while (1) {
				...
				if (ret[0] > 0) {
					...
					if (!el.chord)
						el.chord = [];
					let chordName = tokenizer.translateString(ret[1]);
					chordName = chordName.replace(/;/g, "\n");
					let addedChord = false;
					for (let ci = 0; ci < el.chord.length; ci++) {
					...
					i += ret[0];
					const ii = tokenizer.skipWhiteSpace(line.substring(i));
					if (ii > 0)
			...
			if (ret[0] > 0) {
				...
				// handle chords.
				if (line[i] === '[') {
					const chordStartChar = i;
					i++;
					let chordDuration = null;
					let rememberEndBeam = false;

					let done = false;
					while (!done) {
						const accent = letter_to_accent(line, i);
						if (accent[0] > 0) {
							i += accent[0];
						}

						const chordNote = getCoreNote(line, i, {}, false);
					...
					// Single pitch
					const el2 = {};
					const core = getCoreNote(line, i, el2, true);
					...
					if (core !== null) {
						...
						multilineVars.addFormattingOptions(el, tune.formatting, 'note');
						const succeeded = tuneBuilder.appendElement('note', startOfLine+startI, startOfLine+i, el);
						if (!succeeded) {
							this.startNewLine()
							tuneBuilder.appendElement('note', startOfLine+startI, startOfLine+i, el);
						}
...
var letter_to_chord = function(line, i) {
	if (line[i] === '"')
	{
		const chord = tokenizer.getBrackettedSubstring(line, i, 5);
		if (!chord[2])
		...
		if (chord[0] > 0 && chord[1].length > 0 && chord[1][0] === '^') {
			...
			// @-15,5.7		
			chord[1] = chord[1].substring(1);
			const x = tokenizer.getFloat(chord[1]);
			if (x.digits === 0){
			...
			chord[1] = chord[1].substring(1);
			const y = tokenizer.getFloat(chord[1]);
			if (y.digits === 0){
			...
			const ws = tokenizer.skipWhiteSpace(chord[1]);
...
var letter_to_grace =  function(line, i) {
	...
	if (line[i] === '{') {
		...
		const gra = tokenizer.getBrackettedSubstring(line, i, 1, '}');
		if (!gra[2])
		...
		while (ii < gra[1].length) {
			let acciaccatura = false;
			if (gra[1][ii] === '/') {
				acciaccatura = true;
				ii++;
			}
			const note = getCoreNote(gra[1], ii, {}, false);
			if (note !== null) {
...
function durationOfMeasure(multilineVars) {
	...
	const meter = multilineVars.origMeter;
...
var letter_to_accent = function(line, i) {
	let macro = multilineVars.macros[line[i]];
...
var letter_to_bar = function(line, curr_pos) {
	const ret = tokenizer.getBarLine(line, curr_pos);
	if (ret.len === 0)
		return [0,""];
	if (ret.warn) {
	...
	if (line[curr_pos+ret.len] === '"' && line[curr_pos+ret.len-1] === '[') {
		const ending = tokenizer.getBrackettedSubstring(line, curr_pos+ret.len, 5);
		return [ret.len+ending[0], ret.token, ending[1]];
	}
	const retRep = tokenizer.getTokenOf(line.substring(curr_pos+ret.len), "1234567890-,");
...
MusicParser.prototype.startNewLine = function() {
	const params = { startChar: -1, endChar: -1};
	...
	const scoreTranspose = multilineVars.currentVoice ? multilineVars.currentVoice.scoreTranspose : 0;
	...
	if (multilineVars.key.explicitAccidentals) {
		for (let i = 0; i < multilineVars.key.explicitAccidentals.length; i++) {
			let found = false;
			for (let j = 0; j < params.key.accidentals.length; j++) {
	...
	if (multilineVars.currentVoice) {
		const staff = multilineVars.staves[multilineVars.currentVoice.staffNum];
		...
		const voices = Object.keys(multilineVars.voices)
		for (let mv = 0; mv < voices.length; mv++) {
...
var getCoreNote = function(line, index, el, canHaveBrokenRhythm) {
	...
	const isComplete = function(state) {
		...
		switch(line[index]) {
			...
			case '/':
				if (state === 'octave' || state === 'duration') {
					const fraction = tokenizer.getFraction(line, index);
					...
					const num = tokenizer.getNumber(line, index);
			...
			case '<':
				if (isComplete(state)) {
					if (canHaveBrokenRhythm) {
						const br2 = getBrokenRhythm(line, index);
