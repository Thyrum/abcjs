...
const ParseHeader = function(tokenizer, warn, multilineVars, tune, tuneBuilder) {
	...
	this.setMeter = function(line) {
		...
		{
			const tokens = tokenizer.tokenize(line, 0, line.length);
	...
	this.calcTempo = function(relTempo) {
		...
		for (let i = 0; i < relTempo.duration; i++)
	...
	this.setTempo = function(line, start, end, iChar) {
		...
		try {
			const tokens = tokenizer.tokenize(line, start, end);
	...
	this.letter_to_inline_header = function(line, i, startLine)
	{
		...
		const ws = tokenizer.eatWhiteSpace(line, i);
		i +=ws;
		if (line.length >= i+5 && line[i] === '[' && line[i+2] === ':') {
			const e = line.indexOf(']', i);
			const startChar = multilineVars.iChar + i;
			const endChar = multilineVars.iChar + e + 1;
			switch(line.substring(i, i+3))
			{
				...
				case "[Q:":
					if (e > 0) {
						const tempo = this.setTempo(line, i+3, e, multilineVars.iChar);
	...
	this.parseHeader = function(line) {
		const field = metaTextHeaders[line[0]];
		const origSize = line.length-2
		let restOfLine = tokenizer.translateString(tokenizer.stripComment(line.substring(2)))
		if (field === 'unalignedWords' || field === 'notes') {
			...
			const startChar = multilineVars.iChar;
