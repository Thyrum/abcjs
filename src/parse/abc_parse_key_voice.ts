...
(function() {
	...
	const calcMiddle = function(clef, oct) {
	...
	const parseMiddle = function(str) {
		let i = 0;
		let p = str[i++];
		if (p === '^' || p === '_')
			p = str[i++];
	...
	const normalizeAccidentals = function(accs) {
	...
	const setCurrentVoice = function(id) {
	...
	parseKeyVoice.parseVoice = function(line, i, e) {
		...
		const addNextTokenToVoiceInfo = function(id, name, type) {
			const attr = tokenizer.getVoiceToken(line, start, end);
			if (attr.warn !== undefined)
				warn("Expected value for " + name + " in voice: " + attr.warn, line, start);
			else if (attr.err !== undefined)
		...
		const addNextNoteTokenToVoiceInfo = function(id, name) {
		...
		if (multilineVars.voices[id].staffNum === undefined) {
			...
			let vi = 0;
			for(const v in multilineVars.voices) {
		...
		const s = multilineVars.staves[multilineVars.voices[id].staffNum];
		if (!multilineVars.score_is_present)
			s.numVoices++;
