...
const parseCommon = require('./abc_common');
const parseDirective = require('./abc_parse_directive');
const ParseHeader = require('./abc_parse_header');
const ParseMusic = require('./abc_parse_music');
const Tokenizer = require('./abc_tokenizer');
const wrap = require('./wrap_lines');
const chordGrid = require('./chord-grid');

const Tune = require('../data/abc_tune');
const TuneBuilder = require('../parse/tune-builder');

const Parse = function() {
	"use strict";
	const tune = new Tune();
	const tuneBuilder = new TuneBuilder(tune);
	let tokenizer;
	...
	const multilineVars = {
		...
		duplicateStartEndingHoldOvers: function() {
			...
			for (const key in this.inTieChord) {
		...
		restoreStartEndingHoldOvers: function() {
			...
			for (const key in this.endingHoldOver.inTieChord) {
	...
	const addWarning = function(str) {
		if (!multilineVars.warnings)
			multilineVars.warnings = [];
		multilineVars.warnings.push(str);
	};

	const addWarningObject = function(warningObject) {
	...
	const warn = function(str, line, col_num) {
		if (!line) line = " ";
		let bad_char = line[col_num];
		if (bad_char === ' ' || !bad_char)
			bad_char = "SPACE";
		const clean_line = encode(line.substring(col_num - 64, col_num)) + '<span style="text-decoration:underline;font-size:1.3em;font-weight:bold;">' + bad_char + '</span>' + encode(line.substring(col_num + 1).substring(0,64));
	...
	const addWords = function(line, words) {
	...
	const addSymbols = function(line, words) {
		if (words.indexOf('\x12') >= 0) {
	...
	const parseLine = function(line) {
		...
		const ret = header.parseHeader(line);
		if (ret.regular)
			music.parseMusic(line);
		if (ret.newline)
			music.startNewLine();
	...
	this.parse = function(strTune, switches, startPos) {
		...
		try {
			...
			while (line) {
				...
				const wasInHeader = multilineVars.is_in_header;
				parseLine(line);
