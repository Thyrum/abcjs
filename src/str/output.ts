const keyAccidentals = require("../const/key-accidentals");
const { relativeMajor, transposeKey, relativeMode, isLegalMode } = require("../const/relative-major");
const transposeChordName = require("../parse/transpose-chord")
...
(function () {
	...
	function changeAllKeySigs(abc, steps) {
		...
		for (let i = 1; i < arr.length; i++) {
			...
			if (match) {
				const start = count + 2 + match[1].length // move past the 'K:' and optional white space
				const mode = isLegalMode(match[5]) ? match[5]: ''
				const key = match[2] + match[3] + match[4] + mode // key name, accidental, optional space, and mode
				const destinationKey = newKey({ root: match[2], acc: match[3], mode: mode }, steps)
	...
	function transposeVoices(abc, voices, key, steps) {
		let changes = [];
		const destinationKey = newKey(key, steps)
		...
		for (let i = 0; i < voice.length; i++) {
			...
			if (el.el_type === 'note' && el.pitches) {
				...
				for (let j = 0; j < pitchArray.length; j++) {
					const note = parseNote(pitchArray[j].note, keyRoot, keyAccidentals, measureAccidentals)
					if (note.acc)
				...
				if (el.gracenotes) {
					for (let g = 0; g < el.gracenotes.length; g++) {
						const grace = parseNote(el.gracenotes[g].name, keyRoot, keyAccidentals, measureAccidentals)
						if (grace.acc)
							measureAccidentals[grace.name.toUpperCase()] = grace.acc
						const newGrace = transposePitch(grace, destinationKey, letterDistance, measureAccidentals)
						if (newGrace.acc)
	...
	function parseNote(note, keyRoot, keyAccidentals, measureAccidentals) {
		...
		const reg = note.match(regPitch)
		// reg[1] : "__", "_", "", "=", "^", or "^^"
		// reg[2] : A-G a-g
		// reg[3] : commas or apostrophes
		const name = reg[2].toUpperCase()
		...
		if (name === reg[2]) // See if it is a capital letter and subtract an octave if so.
			oct--;
		const currentAcc = measureAccidentals[name] || keyAccidentals[name] || "=" //  use the key accidentals if they exist, but override with the measure accidentals, and if neither of them exist, use a natural.
	...
	function replaceGrace(abc, start, end, newGrace, index) {
		...
		const regPreBrace = /([^\{]*)/
		const regPreNote = /(\/*)/
		const match = note.match(new RegExp(regPreBrace.source + regOpenBrace.source + regPreNote.source + regOptionalNote.source +
		...
		if (match) {
			...
			let count = 1 + match[1].length // one character for the open brace, and whatever comes before the brace
