...
function createKeyReverse() {
	...
	for (let i = 0 ; i < allKeys.length; i++) {
		const keyObj = keys[allKeys[i]]
		keyReverse[allKeys[i].toLowerCase()] = allKeys[i];
		for (let j = 0; j < keyObj.modes.length; j++) {
			const mode = keyObj.modes[j].toLowerCase()
...
function relativeMajor(key) {
	...
	let mode = key.toLowerCase().match(/([a-g][b#]?)(maj|ion|min|aeo|mix|dor|phr|lyd|loc|m)?/);
	if (!mode || !mode[2])
		return key;
	mode = mode[1] + mode[2]
	const maj = keyReverse[mode]
	if (maj)
		return maj;
	return key;
}
...
function relativeMode(majorKey, mode) {
	...
	if (mode === '')
		return majorKey;
	const match = mode.toLowerCase().match(/^(maj|ion|min|aeo|mix|dor|phr|lyd|loc|m)/);
	if (!match)
		return majorKey
	const regMode = match[1]
	for (let i = 0; i < group.modes.length; i++) {
		const thisMode = group.modes[i]
...
function transposeKey(key, steps) {
	...
	const fromC = (match.stepsFromC + steps) % 12;
	for (let i = 0;  i < Object.keys(keys).length; i++) {
		const k = Object.keys(keys)[i]
