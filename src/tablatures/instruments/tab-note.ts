const { noteToMidi, midiToNote } = require('../../synth/note-to-midi');
...
function cloneNote(self) {
	const newNote = self.name;
	const newTabNote = new TabNote(newNote);
...
TabNote.prototype.checkKeyAccidentals = function (accidentals, measureAccidentals) {
	...
	if (measureAccidentals[this.name.toUpperCase()]) {
		...
		const curNote = this.name;
		for (let iii = 0; iii < accidentals.length; iii++) {
			const curAccidentals = accidentals[iii];
...
TabNote.prototype.emitNoAccidentals = function () {
	...
	for (let ii = 0; ii < this.isQuoted; ii++) {
		returned += "'";
	}
	for (let jj = 0; jj < this.hasComma; jj++) {
		returned += ",";
	}
	return returned;
};
...
TabNote.prototype.emit = function () {
	...
	for (let ii = 1; ii <= this.hasComma; ii++) {
		returned += ',';
	}

	if (this.isLower) {
		returned = returned.toLowerCase();
		for (let jj = 1; jj <= this.isQuoted; jj++) {
			returned += "'";
		}
	}
	return returned;
