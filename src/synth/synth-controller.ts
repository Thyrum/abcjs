const CreateSynthControl = require('./create-synth-control');
const CreateSynth = require('./create-synth');
const TimingCallbacks = require('../api/abc_timing_callbacks');
...
function SynthController() {
	...
	self.go = function () {
		self.isLoading = true;
		const millisecondsPerMeasure = self.visualObj.millisecondsPerMeasure() * 100 / self.warp;
	...
	self.setWarp = function (newWarp) {
		if (parseInt(newWarp, 10) > 0) {
			...
			const wasPlaying = self.isStarted;
			const startPercent = self.percent;
			self.destroy();
			self.isStarted = false;
			return self.go().then(function () {
