...
function Repeats(voice) {
	...
	this.addBar = function(elem) {
		...
		const thisIndex = voice.length - 1
		...
		const startEnding = elem.startEnding ? startEndingNumbers(elem.startEnding) : undefined
		if (isEndRepeat) {
	...
	this.resolveRepeats = function() {
		...
		const repeatInstructions = [] // { common: { start: number, end: number }, endings: Array<{start:number, end:number> }
		...
		for (let i = 0; i < this.sections.length; i++) {
			...
			switch (section.type) {
				case "startRepeat":
					...
					// insert those items.
					if (currentRepeat) {
						let lastUsed = currentRepeat.common.end
						if (currentRepeat.endings) {
							for (e = 0; e < currentRepeat.endings.length; e++) {
		...
		const output = []
		let lastEnd = -1
		for (let r = 0; r < repeatInstructions.length; r++) {
			const instructions = repeatInstructions[r]
			if (!instructions.endings) {
				...
				for (e = 0; e < instructions.endings.length; e++) {
					const ending = instructions.endings[e]
...
function startEndingNumbers(startEnding) {
	...
	if (startEnding.indexOf(',') > 0) {
		...
		endings = startEnding.split('-')
		const se = parseInt(endings[0],10)
		const ee = parseInt(endings[1],10)
		for (i = se; i <= ee; i++) {
			nums.push(i)
		}
	} else {
