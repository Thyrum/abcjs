const getNote = require('./load-note');
const createNoteMap = require('./create-note-map');
...
const pitchToNoteName = require('./pitch-to-note-name');
const instrumentIndexToName = require('./instrument-index-to-name');
const downloadBuffer = require('./download-buffer');
const placeNote = require('./place-note');
...
function CreateSynth() {
	...
	self.init = function(options) {
		...
		let startTime = activeAudioContext().currentTime;
		...
		if (params.programOffsets)
			...
			self.programOffsets = {};
		let p = params.fadeLength !== undefined ? parseInt(params.fadeLength,10) : NaN;
	...
	self.prime = function() {
		...
		const fadeTimeSec = self.fadeLength/1000;
		...
		return new Promise(function(resolve, reject) {
			try {
				...
				const tempoMultiplier = self.millisecondsPerMeasure / 1000 / self.meterSize;
				...
				const totalSamples = Math.floor(activeAudioContext().sampleRate * self.duration);

				// There might be a previous run that needs to be turned off.
				self.stop();

				const noteMapTracks = createNoteMap(self.flattened);
				...
				if (self.options.swing) {
					...
					// proper place.
					const pickupLength = self.options.drumIntro ? 0 : self.pickupLength
				...
				const panDistances = setPan(noteMapTracks.length, self.pan);
				...
				for (let key2 = 0; key2 < Object.keys(uniqueSounds).length; key2++) {
					const k = Object.keys(uniqueSounds)[key2];
					let parts = k.split(":");
					const cents = parts[6] !== undefined ? parseFloat(parts[6]) : 0;
				...
				function resolveData(me) {
					const duration = me && me.audioBuffers && me.audioBuffers.length > 0 ? me.audioBuffers[0].duration : 0;
	...
	self.start = function() {
		...
		const resumePosition = self.pausedTimeSec ? self.pausedTimeSec : 0;
	...
	self.stop = function() {
		...
		self.directSource = [];
		const elapsed = activeAudioContext().currentTime - self.startTimeSec;
		return elapsed;
	};
