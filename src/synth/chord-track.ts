...
const ChordTrack = function ChordTrack(numVoices, chordsOff, midiOptions, meter) {
	this.chordTrack = [];
...
ChordTrack.prototype.setRhythmHead = function (isRhythmHead, elem) {
	...
	if (isRhythmHead) {
		if (this.lastChord && this.lastChord.chick) {
			for (let i2 = 0; i2 < this.lastChord.chick.length; i2++) {
				const note2 = Object.assign({},elem.pitches[0]);
...
ChordTrack.prototype.interpretChord = function (name) {
	...
	let bass2 = bass - 5;	// The alternating bass is a 4th below
	let chick;
	if (name.length === 1)
	...
	if (arr.length === 2) {
		const explicitBass = this.basses[arr[1].substring(0, 1)];
		if (explicitBass) {
			const bassAcc = arr[1].substring(1);
			const bassShift = { '#': 1, '♯': 1, 'b': -1, '♭': -1 }[bassAcc] || 0;
...
ChordTrack.prototype.chordNotes = function (bass, modifier) {
	...
	let intervals = chordIntervals[modifier];
	if (!intervals) {
...
ChordTrack.prototype.chordTrackEmpty = function () {
	let isEmpty = true;
	for (let i = 0; i < this.chordTrack.length && isEmpty; i++) {
...
ChordTrack.prototype.resolveChords = function (startTime, endTime) {
	...
	const currentChordsExpanded = expandCurrentChords(this.currentChords, 8*num/den, beatLength)
	...
	for (var p = 0; p < minLength; p++) {
		...
		const pitches = resolvePitch(currentChordsExpanded[p], type, firstBoom, newBass)
		if (isBoom)
			firstBoom = false
...
ChordTrack.prototype.processChord = function (elem) {
	...
	if (chord) {
		...
		if (c) {
			...
			this.lastChord = c;
			const barBeat = calcBeat(this.lastBarTime, timeToRealTime(elem.time));
...
function resolvePitch(currentChord, type, firstBoom, newBass) {
	...
	const numChordNotes = currentChord.chick.length
	if (type.indexOf('chick') >= 0) {
		for (let i = 0; i < numChordNotes; i++)
