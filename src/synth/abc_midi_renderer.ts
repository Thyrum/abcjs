...
const centsToFactor = require("./cents-to-factor");
let rendererFactory;
...
(function() {
	...
	const HALF_STEP = 4096; // For the pitch wheel - (i.e. the distance from C to C#)
	Midi.prototype.startNote = function(pitch, loudness, cents) {
		...
		if (cents) {
			...
			const bend = Math.round(centsToFactor(cents)*HALF_STEP);
	...
	Midi.prototype.embed = function(parent, noplayer) {
		...
		const embed = setAttributes(document.createElement('embed'), {
			src: data,
	...
	function keySignature(key) {
		...
		const sig = flatCount !== 256 ? toHex(flatCount, 2) : toHex(sharpCount, 2);
	...
	function timeSignature(time) {
		...
		let hex = "%00%FF%58%04" + toHex(time.num,2);
