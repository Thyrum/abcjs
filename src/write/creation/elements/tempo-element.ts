...
const AbsoluteElement = require('./absolute-element').default;
const RelativeElement = require('./relative-element').default;

const TempoElement = function TempoElement(tempo, tuneNumber, createNoteHead) {
...
TempoElement.prototype.createNote = function (createNoteHead, tempo, tuneNumber) {
	...
	const duration = tempo.duration[0]; // TODO when multiple durations
	...
	const ret = createNoteHead(absElem, note, { verticalPos: 0 }, // This is just temporary: we'll offset the vertical positioning when we get the actual vertical spot.
