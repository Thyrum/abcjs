...
const TieElem = function TieElem(options) {
	this.type = "TieElem";
...
TieElem.prototype.calcSlurY = function () {
	if (this.anchor1 && this.anchor2) {
		...
		const beamInterferes = this.anchor2.parent.beam && this.anchor2.parent.beam.stemsUp && this.anchor2.parent.beam.elems[0] !== this.anchor2.parent;
		const midPoint = (this.anchor2.highestVert + this.anchor2.pitch) / 2;
		...
		if (this.anchor1.scalex === 1) { // Need a way to tell if this is a grace note - if so then keep the slur as close as possible. TODO-PER-HACK: this should be more declaratively determined.
			const hasBeam1 = !!this.anchor1.parent.beam
			const hasBeam2 = !!this.anchor2.parent.beam
			if (hasBeam1) {
				const isLastInBeam = this.anchor1.parent === this.anchor1.parent.beam.elems[this.anchor1.parent.beam.elems.length-1]
				if (!isLastInBeam) {
					if (this.above)
			...
			if (hasBeam2) {
				const isFirstInBeam = this.anchor2.parent === this.anchor2.parent.beam.elems[0]
				if (!isFirstInBeam) {
					if (this.above)
...
TieElem.prototype.getYBounds = function () {
	const lineStartX = 10 // TODO-PER: I'm not sure where to get this number from but it probably doesn't matter much
	const lineEndX = 1000 // TODO-PER: I'm not sure where to get this number from but it probably doesn't matter much
	if (this.isTie) {
