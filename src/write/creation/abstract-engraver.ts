...
const BeamElem = require('./elements/beam-element');
const BraceElem = require('./elements/brace-element');
const createClef = require('./create-clef');
const createKeySignature = require('./create-key-signature');
const createNoteHead = require('./create-note-head');
const createTimeSignature = require('./create-time-signature');
const Decoration = require('./decoration');
const EndingElem = require('./elements/ending-element');
...
const StaffGroupElement = require('./elements/staff-group-element');
const TempoElement = require('./elements/tempo-element');
const TieElem = require('./elements/tie-element');
const TripletElem = require('./elements/triplet-element');
const VoiceElement = require('./elements/voice-element');
const addChord = require('./add-chord');
const pitchesToPerc = require('../../synth/pitches-to-perc')

const parseCommon = require('../../parse/abc_common');

const getDuration = function (elem) {
	let d = 0;
	if (elem.duration) {
		d = elem.duration;
	}
...
const AbstractEngraver = function (getTextSize, tuneNumber, options) {
...
const ledgerLines = function (abselem, minPitch, maxPitch, isRest, symbolWidth, additionalLedgers, dir, dx, scale) {
...
AbstractEngraver.prototype.addGraceNotes = function (elem, voice, abselem, notehead, stemHeight, isBagpipes, roomtaken) {
	...
	const graceScaleStem = 3.5 / 5; // TODO-PER: empirically found constant.
	...
	for (i = 0; i < elem.gracenotes.length; i++) {
		...
		if (elem.gracenotes[i].acciaccatura) {
			const pos = elem.gracenotes[i].verticalPos + 7 * gracescale;        // the same formula that determines the flag position.
			const dAcciaccatura = gracebeam ? 5 : 6;        // just an offset to make it line up correctly.
...
AbstractEngraver.prototype.addNoteToAbcElement = function (abselem, elem, dot, stemdir, style, zeroDuration, durlog, nostem, voice) {
	let dotshiftx = 0; // room taken by chords with displaced noteheads which cause dots to shift
	let noteHead;
	let roomTaken = 0;
	...
	for (p = 0; p < elem.pitches.length; p++) {
		...
		const isTopWhenStemIsDown = (stemdir === "up" || dir === "up") && p === 0;
		const isBottomWhenStemIsUp = (stemdir === "down" || dir === "down") && p === pp - 1;
	...
	if (hasStem) {
		...
		let p1 = (dir === "down") ? elem.minpitch - stemHeight : elem.minpitch + 1 / 3;
		// PER added stemdir test to make the line meet the note.
		if (p1 > 6 && !stemdir) p1 = 6;
		let p2 = (dir === "down") ? elem.maxpitch - 1 / 3 : elem.maxpitch + stemHeight;
		// PER added stemdir test to make the line meet the note.
		if (p2 < 6 && !stemdir) p2 = 6;
		const dx = (dir === "down" || abselem.heads.length === 0) ? 0 : abselem.heads[0].w;
...
AbstractEngraver.prototype.createNote = function (elem, nostem, isSingleLineStaff, voice) { //stem presence: true for drawing stemless notehead
	...
	let roomtakenright = 0; // room needed to the right of the note
	let symbolWidth = 0;
	let additionalLedgers = []; // PER: handle the case of [bc'], where the b doesn't have a ledger line
	...
	const durlog = Math.floor(Math.log(duration) / Math.log(2)); //TODO use getDurlog
	let dot = 0;
	...
	let durationForSpacing = duration * this.tripletmultiplier;
...
AbstractEngraver.prototype.createBarLine = function (voice, elem, isFirstStaff) {
	...
	if (elem.chord !== undefined) {
		const ret3 = addChord(this.getTextSize, abselem, elem, 0, 0, 0, false, this.germanAlphabet);
	}
