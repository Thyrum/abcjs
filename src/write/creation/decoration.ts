...
const DynamicDecoration = require('./elements/dynamic-decoration');
const CrescendoElem = require('./elements/crescendo-element');
const GlissandoElem = require('./elements/glissando-element');
...
const Decoration = function Decoration() {
...
const closeDecoration = function (voice, decoration, pitch, width, abselem, roomtaken, dir, minPitch, accentAbove) {
	let yPos;
...
const volumeDecoration = function (voice, decoration, abselem, positioning) {
	for (let i = 0; i < decoration.length; i++) {
		switch (decoration[i]) {
			case "p":
			case "mp":
...
const compoundDecoration = function (decoration, pitch, width, abselem, dir) {
	function highestPitch() {
		...
		let pitch = abselem.heads[0].pitch;
...
const stackedDecoration = function (decoration, width, abselem, yPos, positioning, minTop, minBottom, accentAbove) {
	...
	function symbolDecoration(symbol, placement) {
		...
		const height = glyphs.symbolHeightInPitches(symbol) + 1; // adding a little padding so nothing touches.
...
Decoration.prototype.createDecoration = function (voice, decoration, pitch, width, abselem, roomtaken, dir, minPitch, positioning, hasVocals, accentAbove) {
	...
	const hasOne = stackedDecoration(decoration, width, abselem, yPos, positioning.ornamentPosition, this.minTop, minPitch, accentAbove);
	//if (hasOne) {
