const RelativeElement = require('../creation/elements/relative-element').default;
const spacing = require('../helpers/spacing');
const getBarYAt = require('./get-bar-y-at');

const layoutBeam = function (beam) {
	if (beam.elems.length === 0 || beam.allrests) return;

	const dy = calcDy(beam.stemsUp, beam.isgrace); // This is the width of the beam line.

	// create the main beam
	const firstElement = beam.elems[0];
	const lastElement = beam.elems[beam.elems.length - 1];
	let minStemHeight = 0; // The following is to leave space for "!///!" marks.
...
const getDurlog = function (duration) {
...
function calcYPos(average, numElements, stemHeight, asc, firstAveragePitch, lastAveragePitch, isFlat, minPitch, maxPitch, isGrace) {
	const barpos = stemHeight - 2; // (isGrace)? 5:7;
...
function createStems(elems, asc, beam, dy, mainNote) {
	for (let i = 0; i < elems.length; i++) {
		...
		const ovalDelta = 1 / 5;//(isGrace)?1/3:1/5;
		let pitch = furthestHead.pitch + ((asc) ? ovalDelta : -ovalDelta);
		let dx = asc ? furthestHead.w : 0; // down-pointing stems start on the left side of the note, up-pointing stems start on the right side, so we offset by the note width.
		if (!isGrace)
			dx += furthestHead.dx;
		const x = furthestHead.x + dx; // this is now the actual x location in pixels.
...
function createAdditionalBeams(elems, asc, beam, isGrace, dy) {
	...
	for (let i = 0; i < elems.length; i++) {
		...
		let duration = elem.abcelem.duration; // get the duration via abcelem because of triplets
