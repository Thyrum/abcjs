const layoutBeam = require('./beam');
const getBarYAt = require('./get-bar-y-at');
const layoutTriplet = require('./triplet');
...
function moveDecorations(beam) {
	const padding = 1.5; // This is the vertical padding between elements, in pitches.
			...
			for (let i = 0; i < child.children.length; i++) {
				...
				if (el.klass === 'ornament' && el.position !== 'below') {
					if (el.bottom - padding < top) {
						const distance = top - el.bottom + padding; // Find the distance that it needs to move and add a little margin so the element doesn't touch the beam.
						el.bottom += distance;
						el.top += distance;
						el.pitch += distance;
...
function placeInLane(rightMost, relElem) {
	...
	if (xCoords) {
		for (let i = 0; i < rightMost.length; i++) {
			const fits = rightMost[i] < xCoords.left;
			if (fits) {
				if (i > 0)
...
function numAnnotationsBelow(absElem) {
	let count = 0;
	for (let j = 0; j < absElem.children.length; j++) {
		const relElem = absElem.children[j];
		if (relElem.chordHeightBelow)
			count++;
	}
	return count;
...
function setLane(absElems, numLanesAbove, numLanesBelow) {
	for (let i = 0; i < absElems.length; i++) {
		const below = numAnnotationsBelow(absElems[i]);
