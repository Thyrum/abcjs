...
const setUpperAndLowerElements = function (renderer, staffGroup) {
	...
	for (let i = 0; i < staffGroup.staffs.length; i++) {
		...
		for (let j = 0; j < staff.voices.length; j++) {
			...
			const diff = setUpperAndLowerVoiceElements(positionY, voice, renderer.spacing);
			staff.bottom -= diff//
		}
...
function setUpperAndLowerVoiceElements(positionY, voice, spacing) {
	...
	for (i = 0; i < voice.children.length; i++) {
		...
		const bottom = setUpperAndLowerAbsoluteElements(positionY, abselem, spacing);
...
function setUpperAndLowerAbsoluteElements(specialYResolved, element, spacing) {
	...
	for (let i = 0; i < element.children.length; i++) {
		...
		for (const key in element.specialY) { // for each class of element that needs to be placed vertically
...
function setUpperAndLowerTempoElement(positionY, element) {
	...
	if (element.note) {
		const tempoPitch = element.pitch - element.totalHeightInPitches + 1; // The pitch we receive is the top of the allotted area: change that to practically the bottom.
