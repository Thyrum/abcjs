const layoutVoice = require('./voice');
const setUpperAndLowerElements = require('./set-upper-and-lower-elements');
const layoutStaffGroup = require('./staff-group');
const getLeftEdgeOfStaff = require('./get-left-edge-of-staff');
const layoutInGrid = require('./layout-in-grid');
const toTimeAndStaffBased = require("./to-time-and-staff-based");
...
const layout = function (renderer, abctune, width, space, expandToWidest, timeBasedLayout) {
	...
	const timeBased = toTimeAndStaffBased(abctune.lines)
	for (i = 0; i < abctune.lines.length; i++) {
		abcLine = abctune.lines[i];
...
var setXSpacing = function (renderer, width, space, staffGroup, formatting, isLastLine, debug) {
	...
	for (let it = 0; it < 8; it++) { // TODO-PER: shouldn't need multiple passes, but each pass gets it closer to the right spacing. (Only affects long lines: normal lines break out of this loop quickly.)
...
function fixVoiceCollisions(timeBasedLine) {
	for (let s = 0; s < timeBasedLine.length; s++) {
		const timeSlot = timeBasedLine[s]
		...
		for (let z = 0; z < keys.length; z++) {
			const slot = timeSlot[keys[z]] // slot is an array of all the things happening at a particular time
			const lastIndex = slot.length - 1
			if (slot.length > 1) {
				const isRealRest = slot[0].abcelem.rest && slot[0].abcelem.rest.type === 'rest' // weed out invisible rests
				const isRealRest2 = slot[lastIndex].abcelem.rest && slot[lastIndex].abcelem.rest.type === 'rest' // weed out invisible rests
				if (isRealRest && !slot[lastIndex].abcelem.rest) {
					// the first voice has a rest and the second doesn't
					const restTop = slot[0].children.find(function (ch) { return ch.name.includes('rest') })
					const otherTop = closeTop(slot[lastIndex])
					if (restTop) {
						let distance1 = restTop.bottom - otherTop
					...
					const restBottom = slot[lastIndex].children.find(function (ch) { return ch.name.includes('rest') })
					if (restBottom) {
						let distance2 = restBottom.top - closeBottom(slot[0])
...
function closeTop(absElem) {
	if (absElem.children) {
		let max = -90 // This is clearly way lower than the max calculated below
		for (let i = 0; i < absElem.children.length; i++) {
			const child = absElem.children[i]
			if (child.type !== 'chord')
...
function closeBottom(absElem) {
	if (absElem.children) {
		let min = 90 // This is clearly way higher than the min calculated below
		for (let i = 0; i < absElem.children.length; i++) {
			const child = absElem.children[i]
			if (child.type !== 'lyric')
