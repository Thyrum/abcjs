...
const spacing = require('./helpers/spacing');
const AbstractEngraver = require('./creation/abstract-engraver');
const Renderer = require('./renderer');
const FreeText = require('./creation/elements/free-text');
const Separator = require('./creation/elements/separator');
const Subtitle = require('./creation/elements/subtitle');
const TopText = require('./creation/elements/top-text');
const BottomText = require('./creation/elements/bottom-text');
const setupSelection = require('./interactive/selection');
const layout = require('./layout/layout');
const Classes = require('./helpers/classes').default;
const GetFontAndAttr = require('./helpers/get-font-and-attr');
const GetTextSize = require('./helpers/get-text-size');
const draw = require('./draw/draw');
const tablatures = require('../tablatures/abc_tablatures');
const findSelectableElement = require('./interactive/find-selectable-element');
 ...
 *
 */
const EngraverController = function (paper, params) {
	params = params || {};
...
EngraverController.prototype.engraveABC = function (abctunes, tuneNumber, lineOffset) {
	...
	for (let i = 0; i < abctunes.length; i++) {
...
EngraverController.prototype.getMeasureWidths = function (abcTune) {
	...
	this.getTextSize = new GetTextSize(this.getFontAndAttr, this.renderer.paper);
	const origJazzChords = this.jazzchords
	...
	let needNewSection = true;
	for (let i = 0; i < abcTune.lines.length; i++) {
		const abcLine = abcTune.lines[i];
		if (abcLine.staff) {
			if (needNewSection) {
				section = {
					left: 0,
			...
			if (abcLine.staffGroup.voices.length > 0) {
				const voice = abcLine.staffGroup.voices[0];
				let foundNotStaffExtra = false;
				let lastXPosition = 0;
				for (let k = 0; k < voice.children.length; k++) {
					const child = voice.children[k];
...
EngraverController.prototype.setupTune = function (abcTune, tuneNumber) {
	...
	let scale = abcTune.formatting.scale ? abcTune.formatting.scale : this.scale;
...
EngraverController.prototype.constructTuneElements = function (abcTune) {
	...
	let i;
	let abcLine;
	let hasPrintedTempo = false;
	let hasSeenNonSubtitle = false;
	for (i = 0; i < abcTune.lines.length; i++) {
		...
		if (abcLine.staff) {
			...
			if (hasSeenNonSubtitle) {
				const center = this.width / 2 + this.renderer.padding.left;
...
EngraverController.prototype.engraveTune = function (abcTune, tuneNumber, lineOffset) {
	...
	const origJazzChords = this.jazzchords
	const scale = this.setupTune(abcTune, tuneNumber);
	...
	const maxWidth = layout(this.renderer, abcTune, this.width, this.space, this.expandToWidest, this.timeBasedLayout);
	...
	if (this.expandToWidest && maxWidth > this.width + 1) {
		...
		if ((abcTune.lines)&&(abcTune.lines.length > 0)){
			const nlines = abcTune.lines.length;

			for (let i=0;i<nlines;++i){
				const entry = abcTune.lines[i];
				if (entry.nonMusic){
					if ((entry.nonMusic.rows) && (entry.nonMusic.rows.length > 0)){
						const nRows = entry.nonMusic.rows.length;
						for (let j=0;j<nRows;++j){
							const thisRow = entry.nonMusic.rows[j];
	...
	const ret = draw(this.renderer, this.classes, abcTune, this.width, maxWidth, this.responsive, scale, this.selectTypes, tuneNumber, lineOffset, this.chordGrid);
	this.staffgroups = ret.staffgroups;
	this.selectables = ret.selectables;
	if (this.oneSvgPerLine) {
		const div = this.renderer.paper.svg.parentNode;
...
function splitSvgIntoLines(renderer, output, title, responsive, scale) {
	...
	const source = output.querySelector("svg")
	if (responsive === 'resize')
		output.style.paddingBottom = ''
	const style = source.querySelector("style")
	const width = responsive === 'resize' ? source.viewBox.baseVal.width : source.getAttribute("width")
	const sections = output.querySelectorAll("svg > g") // each section is a line, or the top matter or the bottom matter, or text that has been inserted.
	let nextTop = 0 // There are often gaps between the elements for spacing, so the actual top and height needs to be inferred.
	const wrappers = [] // Create all the elements and place them at once because we use the current svg to get data. It would disappear after placing the first line.
	const svgs = []
	for (let i = 0; i < sections.length; i++) {
		const section = sections[i]
		const box = section.getBBox()
		const gapBetweenLines = box.y - nextTop // take the margin into account
		...
		const svg = duplicateSvg(source)
		const fullTitle = "Sheet Music for \"" + title + "\" section " + (i + 1)
		...
		const viewBoxHeight = renderer.firefox ? height+1 : height
...
EngraverController.prototype.getDim = function (historyEl) {
	...
	if (!historyEl.dim) {
		const box = historyEl.svgEl.getBBox();
