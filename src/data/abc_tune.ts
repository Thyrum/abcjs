...
const sequence = require('../synth/abc_midi_sequencer');
const flatten = require('../synth/abc_midi_flattener');
const delineTune = require("./deline-tune");
...
const Tune = function() {
	...
	this.copyTopInfo = function(src) {
		const attrs = ['tempo', 'title', 'header', 'rhythm', 'origin', 'composer', 'author', 'partOrder'];
		copy(this, src, "metaText", attrs);
		copy(this, src, "metaTextInfo", attrs);
	};

	this.copyBottomInfo = function(src) {
		const attrs = ['unalignedWords',
			'book',
			'source',
			'discography',
			'notes',
	...
	this.getBeatLength = function() {
		...
		const meter = this.getMeterFraction();
		let multiplier = 1;
	...
	function computePickupLength(lines, barLength) {
		...
		for (let i = 0; i < lines.length; i++) {
			if (lines[i].staff) {
				for (let j = 0; j < lines[i].staff.length; j++) {
					for (let v = 0; v < lines[i].staff[j].voices.length; v++) {
						const voice = lines[i].staff[j].voices[v];
						let tripletMultiplier = 1;
						for (let el = 0; el < voice.length; el++) {
							const isSpacer = voice[el].rest && voice[el].rest.type === "spacer";
	...
	this.getPickupLength = function() {
		...
		const pickupLength = computePickupLength(this.lines, barLength);
	...
	this.millisecondsPerMeasure = function(bpmOverride) {
		...
		if (bpmOverride) {
			...
			const tempo = this.metaText ? this.metaText.tempo : null;
		...
		const minutesPerMeasure = beatsPerMeasure / bpm;
		return minutesPerMeasure * 60000;
	};

	this.getBeatsPerMeasure = function() {
		const beatLen = this.getBeatLength();
		const barLen = this.getBarLength();
		...
		for (let i = 0; i < this.lines.length; i++) {
			...
			if (line.staff) {
				for (let j = 0; j < line.staff.length; j++) {
					const meter = line.staff[j].meter;
					if (meter) {
						return meter;
					}
				}
		...
		if (meter) {
			if (meter.type === 'specified') {
				if (meter.value && meter.value.length > 0 && meter.value[0].num.indexOf('+') > 0) {
					const parts = meter.value[0].num.split('+')
					num = 0;
			...
			if (line.staff) {
				for (let j = 0; j < line.staff.length; j++) {
					...
					for (let k = 0; k < staff.voices.length; k++) {
						...
						for (let ii = 0; ii < voice.length; ii++) {
							const elem = voice[ii];
	...
	function addVerticalInfo(timingEvents) {
		...
		let lastEventTop;
		let lastEventBottom;
		for (let e = timingEvents.length - 1; e >= 0; e--) {
			const ev = timingEvents[e];
			if (ev.type === 'bar') {
				ev.top = lastEventTop;
	...
	this.addElementToEvents = function(eventHash, element, voiceTimeMilliseconds, top, height, line, measureNumber, timeDivider, isTiedState, nextIsBar) {
		...
		let realDuration = element.durationClass ? element.durationClass : element.duration;
		...
		if (realDuration > 0) {
			...
			const isTiedToNext = element.startTie;
			if (isTiedState !== undefined) {
				...
				if (!eventHash["event" + voiceTimeMilliseconds]) {
					...
					if (element.abcelem.midiGraceNotePitches && element.abcelem.midiGraceNotePitches.length) {
						...
						for (let j = 0; j < element.abcelem.midiGraceNotePitches.length; j++)
	...
	this.makeVoicesArray = function() {
		...
		const measureNumber = [];
		const tempos = {};
		for (let line = 0; line < this.engraver.staffgroups.length; line++) {
			const group = this.engraver.staffgroups[line];
			if (group && group.staffs && group.staffs.length > 0) {
				const firstStaff = group.staffs[0];
				let middleC = firstStaff.absoluteY;
				const top = middleC - firstStaff.top * spacing.STEP;
				const lastStaff = group.staffs[group.staffs.length - 1];
				middleC = lastStaff.absoluteY;
				const bottom = middleC - lastStaff.bottom * spacing.STEP;
				...
				for (let v = 0; v < voices.length; v++) {
					...
					if (measureNumber[v] === undefined)
						measureNumber[v] = 0;
					const elements = voices[v].children;
	...
	this.setupEvents = function(startingDelay, timeDivider, startingBpm, warp) {
		...
		const time = startingDelay;
		let isTiedState;
		let nextIsBar = true;
		const voices = this.makeVoicesArray();
		let maxVoiceTimeMilliseconds = 0;
		for (let v = 0; v < voices.length; v++) {
			let voiceTime = time;
			let voiceTimeMilliseconds = Math.round(voiceTime * 1000);
			let startingRepeatElem = 0;
			let endingRepeatElem = -1;
			const elements = voices[v];
			var bpm = startingBpm;
			timeDivider = this.getBeatLength() * bpm / 60;
			let tempoDone = -1;
			for (let elem = 0; elem < elements.length; elem++) {
				let thisMeasure = elements[elem].measureNumber;
				...
				const element = elements[elem].elem;
				let ret = this.addElementToEvents(eventHash, element, voiceTimeMilliseconds, elements[elem].top, elements[elem].height, elements[elem].line, elements[elem].measureNumber, timeDivider, isTiedState, nextIsBar);
				...
				if (element.type === 'bar') {
					const barType = element.abcelem.type;
					const endRepeat = (barType === "bar_right_repeat" || barType === "bar_dbl_repeat");
					const startEnding = (element.abcelem.startEnding === '1');
					const startRepeat = (barType === "bar_left_repeat" || barType === "bar_dbl_repeat" || barType === "bar_right_repeat");
					if (endRepeat) {
						...
						let lastVoiceTimeMilliseconds = 0;
						tempoDone = -1;
						for (let el2 = startingRepeatElem; el2 < endingRepeatElem; el2++) {
							...
							const element2 = elements[el2].elem;
	...
	this.addUsefulCallbackInfo = function(timingEvents, bpm) {
		const millisecondsPerMeasure = this.millisecondsPerMeasure(bpm);
		for (let i = 0; i < timingEvents.length; i++) {
			const ev = timingEvents[i];
	...
	function addEndPoints(lines, elements) {
		...
		for (let i = 0; i < elements.length-1; i++) {
			const el = elements[i];
			const next = skipTies(elements, i+1);
			if (el.left !== null) {
				// If there is no left element that is because this is a tie so it should be skipped.
				const endX = (next && el.top === next.top) ? next.left : lines[el.line].staffGroup.w;
	...
	this.getBpm = function(tempo) {
		...
		if (tempo) {
			...
			const statedBeatLength = tempo.duration && tempo.duration.length > 0 ? tempo.duration[0] : beatLength;
	...
	this.setTiming = function (bpm, measuresOfDelay) {
		...
		const tempo = this.metaText ? this.metaText.tempo : null;
		const naturalBpm = this.getBpm(tempo);
		let warp = 1;
		if (bpm) {
			if (tempo)
				warp = bpm / naturalBpm;
		...
		const measureLength = this.getBarLength();

		let startingDelay = measureLength / beatLength * measuresOfDelay / beatsPerSecond;
		if (startingDelay)
			startingDelay -= this.getPickupLength() / beatLength / beatsPerSecond;
		const timeDivider = beatLength * beatsPerSecond;
	...
	this.setUpAudio = function(options) {
		if (!options) options = {};
		const seq = sequence(this, options);
