function delineTune(inputLines, options) {
	...
	const currentKey = [];
	const currentClef = [];
	const currentVocalFont = [];
	const currentGChordFont = [];
	const currentTripletFont = [];
	const currentAnnotationFont = [];
	for (let i = 0; i < inputLines.length; i++) {
		const inputLine = cloneLine(inputLines[i]);
		if (inputLine.staff) {
			if (inMusicLine && !inputLine.vskip) {
				...
				for (let s = 0; s < outputLine.staff.length; s++) {
					const inputStaff = inputLine.staff[s];
					const outputStaff = outputLine.staff[s];
					if (inputStaff) {
					...
					if (inputStaff) {
						for (let v = 0; v < outputStaff.voices.length; v++) {
				...
				for (let ii = 0; ii < inputLine.staff.length; ii++) {
...
function objEqual(input, output) {
	...
	const inputValue = JSON.stringify(input, replacer);
	const outputValue = JSON.stringify(output, replacer);
	...
	for (let i = 0; i < keys.length; i++) {
		if (keys[i] !== "staff")
			...
			for (let j = 0; j < line.staff.length; j++) {
				const staff = {};
				const keys2 = Object.keys(line.staff[j]);
				for (let k = 0; k < keys2.length; k++) {
					if (keys2[k] !== "voices")
						staff[keys2[k]] = line.staff[j][keys2[k]];
					else {
						staff.voices = [];
						for (let v = 0; v < line.staff[j].voices.length; v++) {
