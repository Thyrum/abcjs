...
const wrap = require('../parse/wrap_lines');


const resizeDivs = {};
function resizeOuter() {
	let width = window.innerWidth;
	for (const id in resizeDivs) {
		if (resizeDivs.hasOwnProperty(id)) {
			const outer = resizeDivs[id];
			const ofs = outer.offsetLeft;
			width -= ofs * 2;
...
const renderAbc = function(output, abc, parserParams, engraverParams, renderParams) {
...
function doLineWrapping(div, tune, tuneNumber, abcString, params) {
	...
	const ret = wrap.calcLineWraps(tune, widths, params);
	if (ret.reParse) {
