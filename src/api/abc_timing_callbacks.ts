const TimingCallbacks = function(target, params) {
	const self = this;
	if (!params) params = {};
	...
	self.replaceTarget = function(newTarget) {
		if (!params.qpm) {
			const tempo = newTarget.metaText ? newTarget.metaText.tempo : null;
		...
		const meter = newTarget.getMeter()
		let irregularMeter = ''
		...
		self.beatStarts = []
		if (irregularMeter) {
			const measureLength = self.noteTimings[self.noteTimings.length-1].millisecondsPerMeasure
			const numMeasures = self.lastMoment / measureLength
			const parts = irregularMeter.split("+")
			...
			//        3        |    3    |     4.5
			//        4        |    4    |     6
			for (let measureNumber = 0; measureNumber < numMeasures; measureNumber++) {
				const measureStartTs = measureNumber * measureLength
				let subBeatCounter = 0
				for (let kk = 0; kk < parts.length; kk++) {
					const beatLength = parts[kk] // This is either 1 or 1.5 (how many quarter notes in this beat)
					if (self.beatSubdivisions === 1) {
						...
						const numDivisions = beatLength * self.beatSubdivisions
						for (let k = 0; k < Math.floor(numDivisions); k++) {
							const subBeat = k / numDivisions
							const ts = Math.round(measureStartTs + subBeatCounter * self.millisecondsPerBeat)
			...
			for (let j = 0; j < self.totalBeats+1; j++) {
		...
		if (!self.isPaused && self.isRunning) {
			...
			while (self.noteTimings.length > self.currentEvent && self.noteTimings[self.currentEvent].milliseconds < self.currentTime) {
				if (self.eventCallback && self.noteTimings[self.currentEvent].type === 'event') {
					const thisStartTime = self.startTime; // the event callback can call seek and change the position from beneath us.
			...
			if (self.lineEndCallback && self.lineEndTimings.length > self.currentLine && self.lineEndTimings[self.currentLine].milliseconds < self.currentTime && self.currentEvent < self.noteTimings.length) {
				const leftEvent = self.noteTimings[self.currentEvent].milliseconds === self.currentTime ? self.noteTimings[self.currentEvent] : self.noteTimings[self.currentEvent-1]
			...
			if (self.currentTime < self.lastMoment) {
				...
				if (self.currentBeat < self.beatStarts.length && self.beatStarts[self.currentBeat].ts <= self.currentTime) {
					const ret = self.doBeatCallback(timestamp);
					self.currentBeat++
					if (ret !== null)
				...
				if (self.beatCallback) {
					const ret2 = self.doBeatCallback(timestamp);
					self.currentBeat++
					if (ret2 !== null)
			...
			if (self.currentTime >= self.lastMoment) {
				if (self.eventCallback) {
					...
					const promise = self.eventCallback(null);
	...
	self.doBeatCallback = function(timestamp) {
		if (self.beatCallback) {
			...
			if (ev) {
				...
				const offMs = Math.max(0, timestamp-self.startTime-ev.milliseconds); // Offset in time from the last beat
				const gapMs = endMs - ev.milliseconds; // Length of this event in time
				const gapPx = ev.endX - ev.left; // The length in pixels
			...
			if (self.currentBeat < 0 || self.currentBeat >= self.beatStarts.length || !self.beatStarts[self.currentBeat]) {
				...
				const thisStartTime = self.startTime; // the beat callback can call seek and change the position from beneath us.
				self.beatCallback(
	...
	self.setProgress = function(position, units) {
		...
		const oldBeat = self.currentBeat;
